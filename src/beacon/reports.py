...
def _pr_of(incident: dict[str, Any]) -> str | None:
    """The URL of the pull request Beacon opened for this incident, if any."""
    for e in reversed(incident.get("timeline") or []):
        if isinstance(e, dict) and e.get("event") == "pr_opened":
            url = (e.get("detail") or {}).get("url")
...
def postmortem(
    ...
    *,
    ...
    if pr:
        out.append("")
        out.append("## Durable fix")
        out.append("")
        out.append(
            f"Pull request opened by voice after the runtime fix: {pr} "
            "(the rule declared in the template plus this postmortem; "
            "not merged by Beacon)."
        )
    return "\n".join(out)
    ...
    *,
    ...
    if not rows:
        ...
        for i in rows:
            ...
            pr = _pr_of(i)
            if pr:
                lines.append(f"  Durable fix: pull request {pr}")
        prs = [p for p in (_pr_of(i) for i in rows) if p]
        if prs:
            lines.append(
                f"{len(prs)} pull request(s) opened for review; nothing was merged."
            )
    ...
    return {
        ...
        "pull_requests": [p for p in (_pr_of(i) for i in rows) if p],
        "subject": subject,
        "text": "\n".join(lines),
    }
