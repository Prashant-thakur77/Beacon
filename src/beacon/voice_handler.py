...
from beacon import store, tools
...
def _load_reasoning_prompt() -> str:
    ...
    resource = files("beacon").joinpath("prompts/reasoning.txt")
    return resource.read_text(encoding="utf-8")


def _get_config() -> BeaconConfig:
    """Build config from environment (used by both handler entry points)."""
    return BeaconConfig.from_env()
...
def _live_metric_query(slots: dict[str, Any], config: BeaconConfig) -> dict[str, Any]:
...
def _live_log_query(slots: dict[str, Any], config: BeaconConfig) -> dict[str, Any]:
