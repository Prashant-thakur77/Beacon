...
AI-powered log triage and voice assistant for AWS. Beacon pulls CloudWatch logs, identifies anomalous sections using Cordon (embedding-based log anomaly detection), generates a root cause analysis with Amazon Nova, and optionally calls the on-call engineer to walk them through it by phone.
...
Lambda requires container images to live in ECR. This builds the image from the `Dockerfile` and pushes it to an ECR repository in your account:
...
|-----------|---------|-------------|
| `ImageUri` | *required* | ECR container image URI (printed by `make setup-image`) |
...
**Schedule** (`EnableSchedule=true`): Periodic scans of all configured log groups. Best for routine monitoring. Beacon skips the notification on scheduled scans when logs appear healthy.
...
The Lambda handler (`src/beacon/handler.py`) orchestrates the full pipeline: fetch logs, plan token budget, optionally reduce via Cordon, send to Nova for triage, publish to SNS, and trigger the voice pipeline.

The Cordon integration (`src/beacon/analyzer.py`) uses Nova Embeddings on Bedrock for semantic log anomaly detection. No local model download needed.

Nova 2 Lite (`src/beacon/triage.py`) receives the anomalous log sections (or raw logs if they fit) and produces a structured triage report: severity, root cause, affected components, evidence, and next steps.

The predictive pre-fetch (`src/beacon/prefetch.py`) asks Nova 2 Lite what CloudWatch metrics and logs the engineer would investigate next, then executes those queries in parallel and caches the results in DynamoDB.

The voice handler (`src/beacon/voice_handler.py`) provides a dispatcher with two routes: a briefing handler (reads the RCA for the Connect contact flow to pass to Nova Sonic) and a fulfillment handler (answers follow-up questions using the retrieve-then-reason pattern with cached data and Nova 2 Lite). All voice output is delivered through Nova 2 Sonic speech-to-speech.
