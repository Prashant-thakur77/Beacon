...
from beacon.caller import start_voice_call
from beacon.config import BeaconConfig


def test_start_voice_call_success(voice_config: BeaconConfig, monkeypatch):
    monkeypatch.setenv("CONNECT_CONFIG_PARAM", "/beacon/test/connect-config")
    ...
    import beacon.caller

    beacon.caller._connect_config = None
...
def test_start_voice_call_failure(voice_config: BeaconConfig, monkeypatch):
    monkeypatch.setenv("CONNECT_CONFIG_PARAM", "/beacon/test/connect-config")
