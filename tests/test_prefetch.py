...
from beacon.prefetch import execute, plan, run
...
def test_plan_parses_json(voice_config: BeaconConfig, trigger: TriggerInfo, mocker):
...
def test_plan_handles_markdown_fences(
    voice_config: BeaconConfig, trigger: TriggerInfo, mocker
):
...
def test_execute_runs_queries(
    voice_config: BeaconConfig, prefetch_plan: dict[str, Any], mocker
):
    mocker.patch(
        "beacon.tools.query_metrics",
        return_value={
            "namespace": "AWS/RDS",
            "metric_name": "DatabaseConnections",
...
def test_execute_handles_empty_plan(voice_config: BeaconConfig):
...
def test_run_orchestrates_pipeline(
    ...
    mock_put = mocker.patch("beacon.store.update_cached_data")
