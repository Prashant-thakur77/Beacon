...
| Sessions API is region-sharded | `agents.assemblyai.com` is geo-DNS: a browser in India creates its session on the EU cluster, a Lambda in us-east-1 resolves to the US one, and `GET /v1/sessions/{id}` answers `session_not_found` across the boundary. The recordings route asks `agents.eu.` and `agents.us.` in turn (`ASSEMBLYAI_SESSION_HOSTS`). Probably also why `session.resume` from a fresh token was refused. |
