...
8. First confirm the RDS engine version the template defaults to is still creatable (AWS deprecates minors; 16.4 already was):
   ...
   The demo database password is generated by RDS into Secrets Manager (`ManageMasterUserPassword`); there is no password parameter and nothing to type. If the task stops with `ResourceInitializationError ... secretsmanager`, paste the stopped-task reason: the execution role's `DemoDbSecretRead` policy did not take.
